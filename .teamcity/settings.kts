...
object Build : BuildType({
    ...
    steps {
        ...
        script {
            name = "scalastyle-to -codesquad (1)"
            workingDir = "app"
        ...
        script {
            ...
            workingDir = "app"
            scriptContent = """curl -X PUT -F "projectName=knoldus-leaderboard" -F "moduleName=leaderboard" -F "organisation=knoldus inc" -F "file=@/opt/buildagent/work/%system.agent.work.dir%/app/target/scala-2.12/scapegoat-report/scapegoat.xml" -F "registrationKey=%registrationKey%" https://www.getcodesquad.com/api/add/reports"""
        }
        step {
