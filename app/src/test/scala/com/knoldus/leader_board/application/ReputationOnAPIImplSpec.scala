...
class ReputationOnAPIImplSpec extends AnyWordSpecLike with MockitoSugar with Matchers with ScalatestRouteTest {
  ...
  val reputationCountAndReputation = Option(ReputationCountAndReputation(1, 1, 1, 1,1,1, reputations))
  ...
  when(mockFetchReputation.allTimeAndMonthlyContributionCountWithReputation).thenReturn(Option(ReputationCountAndReputation(1, 1, 1, 1,1,1, reputations)))
  "The service" should {
