...
class ReputationOnAPIImplSpec extends AnyWordSpecLike with MockitoSugar with Matchers with ScalatestRouteTest {
  ...
  val reputationCountAndReputation = Option(ReputationWithCount(1, 1, 1, 1,1,1, reputations))
  ...
  when(mockFetchReputation.allTimeAndMonthlyContributionCountWithReputation).thenReturn(Option(ReputationWithCount(1, 1, 1, 1,1,1, reputations)))
  "The service" should {
