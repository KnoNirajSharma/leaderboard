...
class ScriptActorSpec extends TestKit(ActorSystem("ScriptActorSpec")) with ImplicitSender
  ...
  "Script actor" must {
    "not do anything with incorrect message" in {
      val probe = TestProbe()
      val scriptActor = system.actorOf(Props(new ScriptActor(allTimeReputationActorRef, monthlyReputationActorRef,
