...
class KnolderScoreImplSpec extends AnyFlatSpec with MockitoSugar {
  ...
  "calculate score" should "give score of each knolder" in {
    val scorePerKnolder = List(GetScore(1, "Mukesh Gupta", 215), GetScore(2, "Anjali", 205))
    val counts = List(GetContributionCount(1, "Mukesh Gupta", 3, 2, 1, 1, 1, 1), GetContributionCount(2, "Anjali", 1, 2, 1, 1, 1, 1))
