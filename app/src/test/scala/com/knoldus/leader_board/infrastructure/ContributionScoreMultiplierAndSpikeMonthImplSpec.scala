    ...
    "return contribution score multiplier and spike month" in {
      ...
      val insertDynamicScoring: String =
        """
          ...
          |,oscontribution_score_multiplier,conference_score_multiplier,researchpaper_score_multiplier,book_score_multiplier,month,year)
          |values (?,?,?,?,?,?,?,?,?,?)
""".stripMargin
      ...
      preparedStmtOne.setString(9, month)
      preparedStmtOne.setInt(10, year)
      preparedStmtOne.execute
      preparedStmtOne.close()


      val scoreMultiplier = Option(ScoreMultiplier(2, 2, 2, 2, 2, 2, 2, 2, month, year))
