  ...
  override def fetchReputation: List[Reputation] = {
    ...
    val reputation = SQL(
      """SELECT knolder.id,
                ...
                quarterly_reputation.streak AS quarterlyStreak,
                monthly_reputation.score AS monthlyScore,
                monthly_reputation.rank AS monthlyRank
             from knolder
             INNER JOIN all_time_reputation
                ON knolder.id = all_time_reputation.knolder_id
             INNER JOIN monthly_reputation
                ON knolder.id = monthly_reputation.knolder_id
             INNER JOIN quarterly_reputation
                ON knolder.id = quarterly_reputation.knolder_id
          WHERE knolder.active_status =true
          ORDER BY monthly_reputation.score DESC""".stripMargin)
