...
class FetchReputationImpl(config: Config) extends FetchReputation with LazyLogging {
  ...
  override def fetchReputation: List[Reputation] = {
    ...
    SQL("SELECT knolder.id, knolder.full_name, all_time_reputation.score AS allTimeScore, all_time_reputation.rank " +
      ...
      "monthly_reputation.rank AS monthlyRank from knolder INNER JOIN all_time_reputation ON knolder.id = all_time_" +
      "reputation.knolder_id INNER JOIN monthly_reputation ON knolder.id = monthly_reputation.knolder_id INNER JOIN " +
      "quarterly_reputation ON knolder.id = quarterly_reputation.knolder_id  ORDER BY monthly_reputation.score DESC")
