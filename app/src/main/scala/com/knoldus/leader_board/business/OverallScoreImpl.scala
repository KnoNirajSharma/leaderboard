...
class OverallScoreImpl(fetchData: FetchData, storeData: StoreData, updateData: UpdateData) extends OverallScore {
   ...
   */
  override def calculateScore(listOfBlogsAndAuthors: BlogAuthor): List[AuthorScore] = {
   ...
   */
  override def manageScores(listOfScores: List[AuthorScore]): List[Int] = {
