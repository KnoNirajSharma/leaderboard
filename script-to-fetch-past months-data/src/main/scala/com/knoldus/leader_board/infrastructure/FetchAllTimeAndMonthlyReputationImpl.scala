...
class FetchAllTimeAndMonthlyReputationImpl(config: Config) extends LazyLogging with FetchAllTimeAndMonthlyReputation {
  implicit val connection: Connection = DatabaseConnection.connection(config)
  implicit val session: DBSession = DB.readOnlySession()

  /**
   * Fetching all time contribution count of each knolder.
   *
   * @return List of get contribution count of knolders.
   */

  override def fetchAllTimeReputation(month: Int, year: Int): List[GetContributionCount] = {
    logger.info("Fetching all time contribution of each knolder.")
    SQL("""
      SELECT knolder.id,
    knolder.full_name,
    ...
    GROUP BY knolder.id, knolder.wordpress_id, knolder.email_id, knolder.full_name""")
  ...
  override def fetchMonthlyReputation(month: Int, year: Int): List[GetContributionCount] = {
