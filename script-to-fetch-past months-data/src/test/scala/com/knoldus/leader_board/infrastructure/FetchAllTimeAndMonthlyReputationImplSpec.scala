...
@DoNotDiscover
class FetchAllTimeAndMonthlyReputationImplSpec extends DBSpec with BeforeAndAfterEach with MockitoSugar with Matchers {
  implicit val connection: Connection = DatabaseConnection.connection(ConfigFactory.load())

  val fetchAllTimeAndMonthlyReputation = new FetchAllTimeAndMonthlyReputationImpl(ConfigFactory.load)
  ...
  "FetchAllTimeAndMonthlyReputationImpl" should {
...
""".stripMargin
    ...
    "return all time reputation of knolders" in {
      ...
      val allTimeContributionOfKnolders = List(GetContributionCount(1, "Mukesh Gupta",2,2,2,2,2,2,2,2))

      assert(fetchAllTimeAndMonthlyReputation.fetchAllTimeReputation(8,2020) == allTimeContributionOfKnolders)
    }

    "return  monthly reputation of knolders" in {
      ...
      assert(fetchAllTimeAndMonthlyReputation.fetchMonthlyReputation(4,2020) == allTimeContributionOfKnolders)
    }
  }
}
